    ...
    def after_val_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        for data_sample in outputs:
            ...
            for i, task in enumerate(tasks):
                ...
                ax = axs[3 + i]
                ax.imshow(
                    gt_sem_seg, vmin=0, vmax=self.num_classes[task] - 2, cmap='jet', interpolation='nearest')
                ax.set_xticks([])
                ax.set_yticks([])
                ...
                ax.imshow(pred_sem_seg, vmin=0,
                          vmax=self.num_classes[task] - 2, cmap='jet', interpolation='nearest')
                ax.set_xticks([])
                ax.set_yticks([])
    ...
    def after_test_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        for data_sample in outputs:
            ...
            for i, task in enumerate(tasks):
                ...
                gt_sem_seg = gt_sem_seg = data_sample.gt_sem_seg.data.cpu().numpy()[
                    :, :, i].astype(np.float16)
                nan_mask = gt_sem_seg == 255
                nan_mask[np.isnan(gt_sem_seg)] = True 

                pred_sem_seg = data_sample.get(f'pred_sem_seg_{task}').data.cpu().numpy()[
                    0, :, :].astype(np.float16)

                gt_sem_seg[nan_mask] = np.nan
                pred_sem_seg[nan_mask] = np.nan
